import sys
from PySide6.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QTextEdit, QGroupBox, QMessageBox, QSizePolicy, QScrollArea
from PySide6.QtCore import Qt
from PySide6.QtGui import QFont
import logging
from logger_config import setup_logger
from db_manager import DatabaseManager
from gui_windows import ConnectionDialog, AddDataDialog, ViewDataDialog, AlterTableDialog, AdvancedSelectDialog, TextSearchDialog, StringFunctionsDialog, JoinWizardDialog, SubqueryFilterDialog, CustomTypesDialog, SimilarToDialog, AggregationDialog, CaseConstructorDialog, NullFunctionsDialog, AdvancedGroupingDialog, ViewManagementDialog, MaterializedViewManagementDialog, CTEConstructorDialog
...
class BankSystemApp(QMainWindow):
    ...
    def connect_to_database(self, params):
        try:
            ...
            self.db_manager = DatabaseManager(host=params['host'], port=int(params['port']), database=params['database'], user=params['user'], password=params['password'])
    ...
    def drop_schema(self):
        reply = QMessageBox.question(self, 'Подтверждение удаления схемы', 'Вы уверены, что хотите удалить схему bank_system?\n\nЭто действие:\n• Удалит ВСЕ таблицы и данные в схеме bank_system\n• Удалит пользовательские типы (ENUM)\n• Не может быть отменено!\n\nПродолжить?', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
