from PySide6.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QGridLayout, QTextEdit, QComboBox, QMessageBox, QTabWidget, QWidget, QTableWidget, QTableWidgetItem, QHeaderView, QGroupBox, QScrollArea, QCheckBox, QFormLayout, QApplication, QListWidget
...
class ConnectionDialog(QDialog):
    ...
    def on_connect(self):
        self.connection_params = {'host': self.host_edit.text().strip(), 'port': self.port_edit.text().strip(), 'database': self.database_edit.text().strip(), 'user': self.user_edit.text().strip(), 'password': self.password_edit.text()}
        if not all([self.connection_params['host'], self.connection_params['port'], self.connection_params['database'], self.connection_params['user']]):
...
class AddDataDialog(QDialog):
    ...
    def insert_exchange_rate(self):
        try:
            ...
            rate_id = self.db_manager.insert_exchange_rate(base_currency, target_currency, buy_rate, sell_rate, updated_by)
    ...
    def insert_client(self):
        try:
            ...
            client_id = self.db_manager.insert_client(full_name, passport, phone, email, birth_date, is_vip, allowed_ops)
    ...
    def insert_account(self):
        try:
            ...
            account_id = self.db_manager.insert_account(client_id, currency_code, account_number, balance, status)
    ...
    def insert_transaction(self):
        try:
            ...
            trans_id = self.db_manager.insert_transaction(account_id, trans_type, amount, currency_code, exchange_rate, commission, description, employee)
...
class ViewDataDialog(QDialog):
    ...
    def create_currencies_tab(self):
        ...
        self.currencies_table.setHorizontalHeaderLabels(['ID', 'Код', 'Название', 'Символ', 'Активна'])
    ...
    def create_exchange_rates_tab(self):
        ...
        self.rates_table.setHorizontalHeaderLabels(['ID', 'Базовая', 'Целевая', 'Курс покупки', 'Курс продажи', 'Дата', 'Обновил'])
    ...
    def create_clients_tab(self):
        ...
        self.clients_table.setHorizontalHeaderLabels(['ID', 'ФИО', 'Паспорт', 'Телефон', 'Email', 'Дата регистрации', 'Дата рождения', 'VIP', 'Разрешенные операции'])
    ...
    def create_accounts_tab(self):
        ...
        self.accounts_table.setHorizontalHeaderLabels(['ID', 'Клиент', 'Валюта', 'Номер счета', 'Баланс', 'Статус', 'Дата открытия', 'Последняя операция'])
    ...
    def create_transactions_tab(self):
        ...
        self.transactions_table.setHorizontalHeaderLabels(['ID', 'Клиент', 'Счет', 'Тип', 'Сумма', 'Валюта', 'Курс', 'Комиссия', 'Дата', 'Описание', 'Сотрудник'])
        ...
        try:
            ...
            for row_idx, row_data in enumerate(data):
                for col_idx, value in enumerate(row_data):
                    self.currencies_table.setItem(row_idx, col_idx, QTableWidgetItem(str(value)))
    ...
    def load_exchange_rates(self):
        try:
            ...
            for row_idx, row_data in enumerate(data):
                for col_idx, value in enumerate(row_data):
                    self.rates_table.setItem(row_idx, col_idx, QTableWidgetItem(str(value)))
            QMessageBox.information(self, 'Успех', f'Загружено записей: {len(data)}')
        except Exception as e:
            QMessageBox.critical(self, 'Ошибка', f'Не удалось загрузить данные:\n{str(e)}')
    ...
    def load_clients(self):
        try:
            ...
            for row_idx, row_data in enumerate(data):
                for col_idx, value in enumerate(row_data):
                    self.clients_table.setItem(row_idx, col_idx, QTableWidgetItem(str(value)))
            QMessageBox.information(self, 'Успех', f'Загружено записей: {len(data)}')
        except Exception as e:
            QMessageBox.critical(self, 'Ошибка', f'Не удалось загрузить данные:\n{str(e)}')
    ...
    def load_accounts(self):
        try:
            ...
            for row_idx, row_data in enumerate(data):
                for col_idx, value in enumerate(row_data):
                    self.accounts_table.setItem(row_idx, col_idx, QTableWidgetItem(str(value)))
            QMessageBox.information(self, 'Успех', f'Загружено записей: {len(data)}')
        except Exception as e:
            QMessageBox.critical(self, 'Ошибка', f'Не удалось загрузить данные:\n{str(e)}')
    ...
    def load_transactions(self):
        try:
            ...
            data = self.db_manager.get_transactions(trans_type=trans_type, from_date=from_date, to_date=to_date)
            self.transactions_table.setRowCount(len(data))
            for row_idx, row_data in enumerate(data):
                for col_idx, value in enumerate(row_data):
                    self.transactions_table.setItem(row_idx, col_idx, QTableWidgetItem(str(value)))
            QMessageBox.information(self, 'Успех', f'Загружено записей: {len(data)}')
        except Exception as e:
            QMessageBox.critical(self, 'Ошибка', f'Не удалось загрузить данные:\n{str(e)}')
    ...
    def drop_schema(self):
        reply = QMessageBox.question(self, 'Подтверждение', 'Вы уверены, что хотите удалить схему bank_system?\n\nЭто действие удалит ВСЕ данные в схеме bank_system и не может быть отменено!', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            try:
                if self.db_manager.drop_schema():
                    QMessageBox.information(self, 'Успех', 'Схема bank_system успешно удалена.\n\nПриложение может потребоваться перезапустить для полного обновления.')
                    self.logger.info('Схема успешно удалена')
                else:
                    QMessageBox.warning(self, 'Предупреждение', 'Не удалось удалить схему bank_system')
            except Exception as e:
                QMessageBox.critical(self, 'Ошибка', f'Не удалось удалить схему:\n{str(e)}')
...
class AlterTableDialog(QDialog):
    ...
    def drop_column(self):
        try:
            ...
            reply = QMessageBox.question(self, 'Подтверждение', f"Удалить столбец '{col_name}' из таблицы '{table}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if reply == QMessageBox.StandardButton.Yes:
    ...
    def rename_table(self):
        try:
            ...
            if not new_name:
                ...
                return
            reply = QMessageBox.question(self, 'Подтверждение', f"Переименовать таблицу '{old_name}' в '{new_name}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if reply == QMessageBox.StandardButton.Yes:
    ...
    def change_type(self):
        try:
            ...
            if not new_type:
                ...
                return
            reply = QMessageBox.question(self, 'Подтверждение', f"Изменить тип столбца '{column}' на '{new_type}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if reply == QMessageBox.StandardButton.Yes:
    ...
    def drop_constraint(self):
        try:
            ...
            if not name:
                ...
                return
            reply = QMessageBox.question(self, 'Подтверждение', f"Удалить ограничение '{name}' из таблицы '{table}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if reply == QMessageBox.StandardButton.Yes:
...
class AdvancedSelectDialog(QDialog):
    ...
    def execute_query(self):
        try:
            ...
            results, column_names = self.db_manager.execute_advanced_select(table, columns, where_clause, order_by, group_by, having)
...
class TextSearchDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.search_type_combo.addItems(['LIKE (шаблон)', 'ILIKE (регистронезависимый)', '~ (regex)', '~* (regex без учета регистра)', '!~ (не соответствует regex)', '!~* (не соответствует regex без учета регистра)'])
        ...
        controls_group.setLayout(controls_layout)
        layout.addWidget(controls_group)
        info_label = QLabel('LIKE: используйте % (любые символы), _ (один символ)\nPOSIX regex: ^ (начало), $ (конец), . (любой), * (повтор), [A-Z] (класс)')
    ...
    def execute_search(self):
        try:
            ...
            type_map = {'LIKE (шаблон)': 'LIKE', 'ILIKE (регистронезависимый)': 'ILIKE', '~ (regex)': '~', '~* (regex без учета регистра)': '~*', '!~ (не соответствует regex)': '!~', '!~* (не соответствует regex без учета регистра)': '!~*'}
            search_type = type_map[search_type_text]
            if not pattern:
                QMessageBox.warning(self, 'Ошибка', 'Введите шаблон для поиска')
                return
            results, column_names = self.db_manager.execute_text_search(table, column, pattern, search_type)
            self.result_table.setRowCount(len(results))
            self.result_table.setColumnCount(len(column_names))
            self.result_table.setHorizontalHeaderLabels(column_names)
            for row_idx, row_data in enumerate(results):
...
class StringFunctionsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.function_combo.addItems(['UPPER (верхний регистр)', 'LOWER (нижний регистр)', 'SUBSTRING (подстрока)', 'TRIM (убрать пробелы)', 'LTRIM (убрать слева)', 'RTRIM (убрать справа)', 'LPAD (дополнить слева)', 'RPAD (дополнить справа)', 'CONCAT (объединить)', 'LENGTH (длина строки)'])
    ...
    def execute_function(self):
        try:
            ...
            column = self.column_combo.currentText()
            func_text = self.function_combo.currentText()
            func_map = {'UPPER (верхний регистр)': 'UPPER', 'LOWER (нижний регистр)': 'LOWER', 'SUBSTRING (подстрока)': 'SUBSTRING', 'TRIM (убрать пробелы)': 'TRIM', 'LTRIM (убрать слева)': 'LTRIM', 'RTRIM (убрать справа)': 'RTRIM', 'LPAD (дополнить слева)': 'LPAD', 'RPAD (дополнить справа)': 'RPAD', 'CONCAT (объединить)': 'CONCAT', 'LENGTH (длина строки)': 'LENGTH'}
            ...
            results, column_names = self.db_manager.execute_string_function(table, column, func_type, params)
            self.result_table.setRowCount(len(results))
            self.result_table.setColumnCount(len(column_names))
            self.result_table.setHorizontalHeaderLabels(column_names)
            for row_idx, row_data in enumerate(results):
...
class JoinWizardDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.join_type_combo.addItems(['INNER (внутреннее)', 'LEFT (левое)', 'RIGHT (правое)', 'FULL (полное)'])
        ...
        info_label = QLabel('INNER: только совпадающие записи | LEFT: все из 1-й + совпадения из 2-й\nRIGHT: все из 2-й + совпадения из 1-й | FULL: все записи из обеих таблиц')
        info_label.setStyleSheet('background-color: #e7f3ff; padding: 5px; border: 1px solid #b3d9ff;')
        layout.addWidget(info_label)
    ...
    def execute_join(self):
        try:
            ...
            if selected_items:
                ...
                columns_str = '*'
            join_map = {'INNER (внутреннее)': 'INNER', 'LEFT (левое)': 'LEFT', 'RIGHT (правое)': 'RIGHT', 'FULL (полное)': 'FULL'}
            ...
            results, column_names = self.db_manager.execute_join(table1, table2, column1, column2, join_type, columns)
            self.result_table.setRowCount(len(results))
            self.result_table.setColumnCount(len(column_names))
            self.result_table.setHorizontalHeaderLabels(column_names)
            for row_idx, row_data in enumerate(results):
...
class SubqueryFilterDialog(QDialog):
    ...
    def apply_filter(self):
        try:
            ...
            results, column_names = self.db_manager.execute_subquery_filter(main_table, sub_table, operator, main_col, sub_col)
    ...
    def delete_type(self):
        try:
            ...
            if not type_name:
                ...
                return
            reply = QMessageBox.question(self, 'Подтверждение', f'Удалить тип {type_name}?', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
            if reply == QMessageBox.StandardButton.Yes:
    ...
    def apply_search(self):
        try:
            ...
            if not pattern:
                ...
                return
            results, column_names = self.db_manager.execute_text_search(table, column, pattern, operator)
...
class AggregationDialog(QDialog):
    ...
    def apply_aggregation(self):
        try:
            ...
            results, column_names = self.db_manager.execute_aggregation(table, agg_func, agg_column, group_by, having)
...
class CaseConstructorDialog(QDialog):
    ...
    def execute_case(self):
        try:
            ...
            results, column_names = self.db_manager.execute_case_expression(table, case_expr, select_cols)
    ...
    def execute_function(self):
        try:
            ...
            if func_type == 'COALESCE':
                results, column_names = self.db_manager.execute_coalesce_nullif(table, func_type, column, coalesce_values=[column, param], select_cols=select_cols)
                expr = f'COALESCE({column}, {param})'
            else:
                results, column_names = self.db_manager.execute_coalesce_nullif(table, func_type, column, nullif_val1=param, select_cols=select_cols)
...
class AdvancedGroupingDialog(QDialog):
    ...
    def __init__(self, parent, db_manager):
        ...
        self.setMinimumSize(1000, 600)
        self.table_columns = {'currencies': ['currency_id', 'currency_code', 'is_active'], 'exchange_rates': ['base_currency', 'target_currency', 'rate_date'], 'clients': ['client_id', 'is_vip'], 'accounts': ['client_id', 'currency_code', 'account_status'], 'transactions': ['account_id', 'transaction_type', 'currency_code']}
        self.setup_ui()
    ...
    def execute_grouping(self):
        try:
            ...
            results, column_names = self.db_manager.execute_advanced_grouping(table, select_cols, group_type, selected_cols, where, None, order)
...
class ViewManagementDialog(QDialog):
    ...
    def delete_view(self):
        ...
        if not view_name:
            ...
            return
        reply = QMessageBox.question(self, 'Подтверждение удаления', f"Вы уверены, что хотите удалить представление '{view_name}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            try:
...
class MaterializedViewManagementDialog(QDialog):
    ...
    def delete_view(self):
        ...
        if not view_name:
            QMessageBox.warning(self, 'Ошибка', 'Выберите представление для удаления')
            return
        reply = QMessageBox.question(self, 'Подтверждение', f"Вы уверены, что хотите удалить представление '{view_name}'?")
        if reply == QMessageBox.StandardButton.Yes:
            try:
...
class CTEConstructorDialog(QDialog):
    ...
    def add_cte(self):
        ...
        self.ctes[cte_name] = {'table': table_name, 'selected_columns': selected_columns, 'where': where_condition}
